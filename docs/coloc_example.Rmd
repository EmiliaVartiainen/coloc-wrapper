...
R3 FinnGen GWAS files need to be reindexed. Here's the command:

```
tabix I9_VARICVE.gz -S 1 -b 2 -e 2 -s 1
```
...
Rscript extdata/step1_subset_data.R	\
	--file=finngen_R3_I9_VARICVE.gz\
	--region="5:127570567-128570567" \
  --out=finngen_R3_I9_VARICVE_5_127570567-128570567.txt/
```

eQTL dataset has the variant identifier in the format `chr#_chrom_pos_alt_ref`

```
cat finngen_R3_I9_VARICVE_5_127570567-128570567.txt | awk '{print "chr"$1"_"$2"_"$3"_"$4"\t"$0}'  > tmp && mv tmp finngen_R3_I9_VARICVE_5_127570567-128570567.txt
```
...
Rscript extdata/step2_run_coloc.R	\
  --gwas="finngen_R3_I9_VARICVE_5_127570567-128570567.txt" \
	...
	--out="coloc_VARICVE_BLUEPRINT_5_127570567-128570567.txt" \
```
...
```{r, echo=FALSE}
library(DT)
coloc_results <- data.table::fread("coloc_VARICVE_BLUEPRINT_5_127570567-128570567.txt", sep = "\t")
...
library(knitr)
knitr::include_graphics("coloc_VARICVE_BLUEPRINT_5_127570567-128570567_locuscompare_ENSG00000064651.14_5_128187026_128188442.png")
```
