...
run_coloc <- function(top_loci, eqtl, gwas, n, s, endpoint, eqtl_name, tissue, file) {
  ...
  write.table(coloc_df, file = paste0("/Users/eahvarti/Documents/coloc_analysis/Results/GTEx_results/", file), sep = "\t")
  ...
  for (i in 1:nrow(top_loci)) {
    ...
    locus <- paste0(top_loci$chr[i], ":", top_loci$start[i], "-", top_loci$end[i])
    print(locus)
    ...
    if (!is.null(eqtl_region)) {
      ...
      for (j in unique(eqtl_region$molecular_trait_id)) { # Goes through all the genes in eQTL data set and runs coloc for each of them 
        ...
        write(as.matrix(results[1,]), file = paste0("/Users/eahvarti/Documents/coloc_analysis/Results/GTEx_results/", file), sep = "\t", append = TRUE, ncolumns = 11)
      }
    }
  }
    ...
    if (!is.null(eqtl_region)) {
      ...
      for (j in unique(eqtl_region$gene_id)) { # Goes through all the genes in eQTL data set 
        ...
        if (!is.na(gene)) {
          ...
          if (max(H4) > 0) { # Plot the genes that are important with threshold 
            eqtl_subset <- eqtl_region[which(eqtl_region$gene_id == j),] # subset of the eQTL data set based on the gene id 
            names(eqtl_subset) <- paste0(names(eqtl_subset), ".eqtl")
            dat_subset <- eqtl_subset %>% inner_join(gwas_region, by = c("snp.eqtl" = "snp.gwas"))
...
```{r, coloc_analysis}
eQTL_data_name = dplyr::filter(tabix_paths, study == "GENCORD", tissue_label == "LCL")[1,] # Specify which eQTL dataset is going to be used 
eQTL_data_name = dplyr::filter(imported_tabix_paths, qtl_group == "Adipose_Subcutaneous") # Specify which eQTL dataset is going to be used, GTEx data
...
coloc_VARICVE <- run_coloc(top_variants_VARICVE, eQTL_data_name, GWAS_path_VARICVE, n = 207055, s = 17027/207055, endpoint = "Varicose veins", 
                           eqtl_name = "GENCORD", tissue = "LCL", "coloc_VARICVE_results_Adipose_Subcutaneous.txt") 
coloc_ICP <- run_coloc(top_variants_ICP, eQTL_data_name, GWAS_path_ICP, n = 123579, s = 940/123579, endpoint = "Intrahepatic Cholestasis of Pregnancy (ICP)Ê", 
                           eqtl_name = "GENCORD", tissue = "LCL", "coloc_ICP_results_GENCORD_LCL.txt")
...
# Run COLOC to all the GTEx tissues 
for (tissue in unique(imported_tabix_paths$tissue_label)[17:49]) {
  print(tissue)
  eQTL_data_name = dplyr::filter(imported_tabix_paths, tissue_label == tissue) # Specify which eQTL dataset is going to be used, GTEx data
  
  coloc_VARICVE <- run_coloc(top_variants_VARICVE, eQTL_data_name, GWAS_path_VARICVE, n = 207055, s = 17027/207055, endpoint = "Varicose veins", 
                           eqtl_name = "GTEx_V8", tissue = tissue, file = paste0("coloc_VARICVE_results_", 
                                                                                 imported_tabix_paths$qtl_group[which(imported_tabix_paths$tissue_label == tissue)], ".txt"))
  #print(paste("Coloc VARICVE", tissue, "done!"))
  #coloc_ICP <- run_coloc(top_variants_ICP, eQTL_data_name, GWAS_path_ICP, n = 123579, s = 940/123579, endpoint = "Intrahepatic Cholestasis of Pregnancy (ICP)Ê", 
  #                         eqtl_name = "GTEx_V8", tissue = tissue, file = paste0("coloc_ICP_results_", 
  #                                                                               imported_tabix_paths$qtl_group[which(imported_tabix_paths$tissue_label == tissue)], ".txt"))
  #print(paste("Coloc ICP", tissue, "done!"))
}
...
```{r, top_results}
results <- read.csv("/Users/eahvarti/Documents/coloc_analysis/Results/coloc_ICP_results.txt", header = TRUE, sep = "\t")
names(results)[which(names(results) == "Gene_ID")] <- "Ensembl_ID"
